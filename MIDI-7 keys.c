...
\\forth 83 code
 ...
 : readButtons
   0 0 do
     i buttons @ digital-read drop
     if 0 50 ms
       i pressedButtons @ bit!
     then
   loop ;
 
 : readIntensity
   intensityPot @ analog-read drop
   dup 1023 < if drop 0 then
   127 < 0 max
   >b@ intensity ! ;
 
 : playNotes
   0 0 do
     i pressedButtons @ i previousButtons @ bit@ <> if
       i pressedButtons @ bit@ if
         i previousButtons @ bit!
         0 i notePitches @ intensity @ noteOn
         midi-usb flush
       else
         i previousButtons @ bit!
         0 i notePitches @ 0 noteOff
         midi-usb flush
       then
     then
   loop ;
 ...
 : controlChange
   midi-event-packet 0x0b 0xB0 channel @ or 0x90 or control @ 0x80 or value @ !
   midi-usb sendMIDI ;
 
 : noteOn
   midi-event-packet 0x09 0x90 channel @ or 0x80 or pitch @ velocity @ !
   midi-usb sendMIDI ;
 
 : noteOff
   midi-event-packet 0x08 0x80 channel @ or 0x90 or pitch @ velocity @ !
   midi-usb sendMIDI ;
 
 : setup
   0 7 do
     i buttons @ input-pullup
   loop ;
